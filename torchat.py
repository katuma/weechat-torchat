# -*- coding: UTF-8 -*-
# Copyright (C) 2012 Karel Tuma <karel.tuma@gmail.com>
#
...
#
##############################################################################
# Low-overhead TorChat protocol implementation for WeeChat

SCRIPT_NAME    = "torchat"
SCRIPT_AUTHOR  = "Karel Tuma <karel.tuma@gmail.com>"
SCRIPT_VERSION = "1.0"
SCRIPT_LICENSE = "GPL3"
SCRIPT_DESC    = "TorChat protocol for WeeChat"
SCRIPT_COMMAND = SCRIPT_NAME # "tc"

SOCKS_TIMEOUT = 10
ATTEMPT_TIMER = 300
KEEPALIVE_INTERVAL = 120
PING_TIMEOUT = KEEPALIVE_INTERVAL*2
...
HiddenServicePort 11009 127.0.0.1:{HIDSERV_PORT}
...
class TCBuddy():
    def __init__(self, address, torchat, authorized=False):
        self.address = address
        self.authorized = authorized
        self.name = ""
        self.desc = ""
        self.reset(torchat)
    def reset(self, torchat):
        self._gotping = False
        self._torchat = torchat
        self._timer = None
        self._random = str(random.getrandbits(256))
        self._state = 0
        self._cookie_lock = thread.allocate_lock()
    ...
    def start_connect(self):
        ...
        self._state = TC_CONNECTING
        thread.start_new_thread(TCBuddy.connect_thread, (self, ))
    def __getstate__(self):
        d = self.__dict__.copy()
        # stuff prefixed with _ won't get serialized
        for k,v in d.items():
            if k.startswith('_'):
                del d[k]
        return d
    # reset() must becalled for each, to restore app context
    def __setstate__(self, dict):
        self.__dict__.update(dict)
    def send(self,data):
        return self._send.sendall(data)
    def sendLF(self,data):
        return self.send(data.replace("\\", "\\/").replace("\n", "\\n"))

    # this is the sending side. note that receiving on self._send is only
    # for file transfers.
    def connect_thread(self):
        for i in range(1,10):
            try:
                self._send = socket.create_connection(self._torchat.socks, SOCKS_TIMEOUT)
                self.sendall("\x04\x01\x00\x00\x00\x01\x00" + self.address + ".onion\x00")
                assert(self.recv(8)[1] == chr(90))
            except:
                sleep(1)
                continue
        # tor is up, go on
        self.sendLF("ping %s %s" % (self._torchat.address, self._random))
        self.send_cookie()
        # XXX
        self._send.recv(1)
        self._send.close()
    ...
    def send_cookie(self):
        self._cookie_lock.acquire()
        if self._cookie and self._state != TC_CONNECTED:
            self.sendLF("pong %s" % (self._cookie))
            self._cookie = None
            self._state = TC_CONNECTED
        self._cookie_lock.release()
# protocol handlers
    def on_ping(self,data)
        self.address, self.answer = data.split(' ', 1)
    ...
    def on_status(self,data):
        self.status = data
    def on_profile_name(self,data):
        self.name = data
    def on_profile_text(self,data):
        self.desc = data
   
class TorChat(object):
    def __init__(self, hostname, socks, listensock, roster):
        self.socks = socks
        self.buddies = roster
        for b in roster.itervalues(): b.reset()
        ls = listensock
        ls.listen(1024)
        ls.settimeout(ATTEMPT_TIMER)
        lastattempt = 0
        rlist = [ls]
        sockets = {}
        timeouts = {}
        buffers = {}
        while True:
            r,w,x=rselect.select(rlist,[],[], PING_TIMEOUT)
            now=time()
            for s in r:
                if s==nsk:
                    nsk = ls.accept()
                    nsk.setnonblock(True)
                    rlist.append(nsk)
                    timeouts[nsk]=now
                    buffers[nsk]=[]
                else:
                    ...
                    if br >= 0:
                        buf = buffers[nsk]
                        buf.append(got[:br])
                        buf = decodeLF("".join(buf))
                        buffers[nsk]=got[br+1:]
                        cmd, args = buf.split(" ", 1)
                        if (cmd == "ping"):
                            hostname, cookie = args.split(1)
                            sockets[nsk] = self.find_or_create_buddy(address, cookie)
                        else
                            buddy = sockets[nsk]
                            buddy.__dict__["on_"+cmd](args)
                    else:
                        buf.append(got)
            # once in a while, try to connect
            if (lastattempt + PING_TIMEOUT < now()):
                lastattempt = time()
                for b in roster.itervalues():
                    b.start_connect()
    def find_or_create_buddy(self, address, cookie=None):
        if not address in self.buddies:
            self.buddies[address] = TCBuddy(address, self)
        buddy = self.buddies[address]
        buddy.set_cookie(cookie)
        buddy.start_connect()
        return buddy
# public api
...
######## original tc_client.py

try:
    import SocksiPy.socks as socks
except:
    print("Package SocksiPy (pip install socksipy-branch) must be installed to use TorChat protocol.")
    import_ok = False
finally:
...
SOCKS_PORT = 0 # to be filled in
LISTENER_PORT = 0 # to be filled in
...
def myHostname():
    return w.config_get_plugin('own_hostname')

def setMyHostname(h)
    return w.config_set_plugin('own_hostname', h)

def getDataDir():
    return w.config_get_plugin('datadir')

def getProfile():
    return w.config_get_plugin('profile_name','profile_text')
...
class Buddy(object):
    ...
    def getOfflineFileName(self):
        return os.path.join(getDataDir(),self.address + "_offline.txt")
    ...
    def keepAlive(self):
        ...
        if self.conn_out == None:
            ...
            if self.conn_in:
                ...
                if self.count_unanswered_pings < MAX_UNANSWERED_PINGS:
                    self.sendPing()
    ...
    def sendPing(self):
        ...
        #self.random1 = str(random.getrandbits(256))
        ping = ProtocolMsg_ping(self, (myHostname(), self.random1))
        ping.send()
...
class BuddyList(object):
    ...
    def __init__(self, callback, socket=None):
        ...
        for i in range(0,60):
            if (myHostname()):
                if i > 0:
                    print "(1) got hostname %s" % myHostname()
                break
        ...
        for buddy in self.list:
            if buddy.address == myHostname()
                found = True
                self.own_buddy = buddy
                break

        if not found:
            print "(1) adding own hostname %s to list" % myHostname()
            if myHostname() != "":
                self.own_buddy = Buddy(myHostname(),
                                 self,
                                 "myself")
    ...
    def save(self):
        f = open(os.path.join(getDataDir(), "buddy-list.txt"), "w")
        for buddy in self.list:
...
class OutConnection(threading.Thread):
    ...
    def run(self):
        ...
        try:
            ...
            self.socket.setproxy(socks.PROXY_TYPE_SOCKS4, "127.0.0.1", SOCKS_PORT)
...
def startPortableTor():
    ...
    old_dir = os.getcwd()
    if not TOR_LISTENER:
        print "(1) Listener is not up yet"
        return
    ...
    try:
        ...
        if isWindows():
            if os.path.exists(getTorBin()):
                #start the process without opening a console window
                startupinfo = subprocess.STARTUPINFO()
                startupinfo.dwFlags |= 1 #STARTF_USESHOWWINDOW
                f=open("torrc.txt.","w+")
                f.write(TORRC_TEMPLATE.format(**globals()))
                f.close()
                tor_proc = subprocess.Popen((getTorBind()+" -f torrc.txt").split(), startupinfo=startupinfo)
            ...
            if os.path.exists(getTorBin()):
                ...
                f.write(TORRC_TEMPLATE.format(**globals()))
                f.close()
                tor_proc = subprocess.Popen((getTorBin()+" -f .torrc").split())
                tor_pid = tor_proc.pid
...
def torchat_unload_script()
    # XXX
    pass


def torchat_callback(t,d):
    if t==
    

def torchat_start(data,buffer,args)
    global instance
    if not instance:
        w.buffer_new("TorChat","torchat_input_cb", "", "torchat_close_cb","")
        instance = BuddyList(torchat_callback)

if __name__ == "__main__" and import_ok:
    if w.register(SCRIPT_NAME, SCRIPT_AUTHOR, SCRIPT_VERSION, SCRIPT_LICENSE, SCRIPT_DESC, "torchat_unload_script", ""):
        defaults = {
            "torbin" : "tor",
            "datadir" : os.path.join(weechat.info_get("weechat_dir", ""), "torchat"),
            "own_hostname": "",
            "profile_name": "",
            "profile_text": "",
        }
        ...
        for dir in ("tor_data","hidden_service","incoming","downloads"):
            try:
                os.mkdir(os.path.join(getDataDir(), dir))
            except OSError:
                pass

        w.hook_command("torchat", "Start torchat", "No arguments atm", "torchat_start", "")
        w.hook_command("tchat", "Chat with a Torchat buddy", "<buddy>", "buddy: nick of buddy", "", "torchat_tchat", "")
        w.hook_command("tmsg", "Send a message to a Torchat buddy","<buddy>", "buddy: nick of buddy", "", "torchat_tmsg", "")
